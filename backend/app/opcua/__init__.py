...
class _SubscriptionHandler:
    ...
    def __init__(self, node_map: dict[str, str], dispatcher: 'Dispatcher') -> None:
        # node_map: {node_id_str → (station_id, direction)}
    ...
    def datachange_notification(self, node, val, data):
        ...
        if not prev and cur:
            ...
            if task:
                log.info("[OpcUA] task criada: %s", task.id)
...
class OpcUaCallbuttonDriver:
    ...
    async def _monitor(self) -> None:
        ...
        # Todos os stations com opcua_node configurado (callbuttons + APs)
        cb_stations = [
                ...
                async with Client(config.OPCUA_ENDPOINT) as client:
                    ...
                    for s in cb_stations:
                        ...
                        if s.get('opcua_node'):
                            ...
                            node_map[str(node.nodeid)] = (s['id'], 'fwd')
                            nodes.append(node)
                        # nó return
                        if s.get('opcua_ret'):
                            node = client.get_node(s['opcua_ret'])
                            node_map[str(node.nodeid)] = (s['id'], 'ret')
                            nodes.append(node)
                        log.info("  subscribed: %s fwd/ret → %s", s['id'], s['opcua_node'])
