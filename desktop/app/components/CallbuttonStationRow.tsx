...
export function CallbuttonStationRow({ station }: { station: Station }) {
  ...
  async function handleSave() {
    ...
    try {
      ...
      setSaveError(e instanceof FleetApiError ? e.message : 'Save failed')
    } finally {
      setSaving(false)
    }
  }
  ...
  async function handleTest() {
    ...
    try {
      ...
      setTestError(e instanceof FleetApiError ? e.message : 'Test failed')
    } finally {
      setTesting(false)
    }
  }
        ...
        <td className="px-4 py-2">
          <div className="flex items-center gap-1">
            <Input value={node} onChange={e => setNode(e.target.value)} placeholder="ns=2;s=…"
              ...
              disabled={saving} />
            <Button variant="outline" size="sm" onClick={handleSave} disabled={saving || !dirty} title="Save node">
...
function TestResultLine({ res }: { res: OpcuaTestResult }) {
  if (!res.configured) {
    return <div className="text-[#8b949e]">No OPC UA endpoint configured.</div>
  }
  if (res.ok) {
    return (
      <div className="text-[#3fb950]">
        ✅ Read <span className="font-mono text-[#c9d1d9]">{res.node}</span>
        {' → '}<span className="font-mono text-[#e6edf3]">{JSON.stringify(res.value)}</span>
