...
class Module(BasicModule):
    ...
    def run(self, r: SimModule, args):
        ...
        if self.move_action == MoveAction.InitcallGo2QRCenter:
            self.status = r.goPGVRun({"use_down_pgv":not self.upside, "action_name":"callGo2QRCenter", "PGV_ReachDist":0.01})
            ...
            self.status = r.runOdoMove({"move_angle": self.short_rot_angle,  "speed_w":-self.speed_w, "action_name":"ShortRotRightInPlace"})
            ...
            self.status = r.runOdoMove({"move_angle": self.move_angle,  "speed_w":self.speed_w, "action_name":"RotLeftInPlace"})
