    ...
    def run(self, r: SimModule, args):
        ...
        if self.init:
            ...
            if type(args) is dict and "V" in args:
                ...
                self.speed = 0.5
            self.speed_x_rate=args.get("speed_x_rate")
            self.speed_y_rate=args.get("speed_y_rate")
            self.speed_x = self.speed*self.speed_x_rate
            self.speed_y = self.speed*self.speed_y_rate

        # 实时运行
        if self.move_action == 0:
            self.status = r.runOdoMove({"move_dist": self.move_dist,  "speed_x":self.speed_x, "speed_y":self.speed_y, "action_name":"GoStraightForward"})
        elif self.move_action == 1:
            self.status = r.runOdoMove({"move_dist": self.move_dist,  "speed_x":-self.speed_x, "speed_y":-self.speed_y, "action_name":"GoStraightBackward"})
