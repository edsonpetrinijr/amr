...
sys.path.append('/usr/local/etc/.SeerRobotics/rbk/resources/scripts/genetic/syspy/protobuf/arm')
...
class canPassAarch64():
    ...
    def sendCanframe(self, channel, can_id, dlc, extend, can_string: list):
        ...
        # 将 can_string 从字节数组转换为以空格分隔的十六进制字符串
        hex_can_string = ' '.join(format(byte, '02X') for byte in can_string)
        print(f'message send: channel={channel}, can_id={hex(can_id)}, dlc={dlc}, extend={extend}, can_string={hex_can_string}')
